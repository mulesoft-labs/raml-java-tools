...
public class ArrayTypeHandler implements TypeHandler {
    ...
    public TypeName javaClassReference(GenerationContext generationContext, EventType type) {
        return ParameterizedTypeName.get(ClassName.get(List.class), TypeDeclarationType.calculateTypeName(typeDeclaration.items().name(), typeDeclaration.items(), generationContext, type).box());
    }
